...
<template>
  <div
    class="min-h-screen bg-linear-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4"
  >
    <div class="w-full max-w-4xl space-y-8 md:space-y-12">
      ...
      >
        <h1
          class="text-4xl md:text-5xl font-light text-white tracking-wide mb-2"
        >
          Breathe
        </h1>
      ...
      <!-- Breathing Circle -->
      <div class="py-28 md:py-28" v-if="breathing.isActive.value || breathing.totalSessionTime.value > 0">
        <BreathingCircle
          :scale="breathing.circleScale.value"
          :phase="
            breathing.isActive.value || breathing.totalSessionTime.value > 0
              ? breathing.phaseLabel.value
              : 'Ready'
          "
